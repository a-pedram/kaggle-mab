...
def new_bandit(step):
    ...
    if myProbabs[his_last_bandit] > unlessMyProbab and hits[his_last_bandit] <max_hit and his_hits[his_last_bandit]>1:
        if step > 60:
            if his_last_bandit in np.argsort( myProbabs)[10:] :
    ...
    if his_hits[his_last_bandit] == 1:
        if last_reward == 1:
            return last_bandit
        if myProbabs[his_last_bandit]> .48:
            return his_last_bandit
    scores[his_hits == 1] -= 1
