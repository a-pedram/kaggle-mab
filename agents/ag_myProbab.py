...
def new_bandit(step):
    ...
    '''n_lookback = 5
    if my_hits[last_bandit] >= n_lookback :
        n_wins = np.sum(bandits_record[record_index[last_bandit]-n_lookback:record_index[last_bandit],last_bandit])
        n_losses = n_lookback - n_wins
        myProbabs[last_bandit]probab(n_wins, n_losses)
    else:'''
    myProbabs[last_bandit] = probab(wins[last_bandit]+1,losses[last_bandit])
    #print(myProbabs)
...
def agent(obs, conf):
    ...
    if obs.step == 0:        
        ...
        bandits_record = np.zeros([400, conf.banditCount])
