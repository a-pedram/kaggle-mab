...
def new_probab(bandit_rec_c):
    ...
    return ((np.sum(bandit_rec * denomin*k2))/(denomin.sum()))
...
def new_bandit(step):
    ...
    step2 = int(step / 200)
    myProbabs[last_bandit] = (2*decayed_probab(wins[last_bandit],losses[last_bandit],his_hits[last_bandit]) + \
        new_probab(bandits_record[last_bandit, 0:record_index[last_bandit]])) /3
    myProbabs[his_last_bandit] = (2* decayed_probab(wins[his_last_bandit],losses[his_last_bandit],his_hits[his_last_bandit]) + \
        new_probab(bandits_record[his_last_bandit, 0:record_index[his_last_bandit]]) ) / 3
    if myProbabs[his_last_bandit] < probab_step[step2]  and \
        my_hits[his_last_bandit] > n_decision and \
        myProbabs.max() > (probab_step[step2] * 1.1): 
