...
def new_bandit(step):
    ...
    exploratio_score[his_hits == 0] = -0.5 # -0.05
    exploratio_score[his_hits == 1] = -1
    #exploratio_score += 1 /(my_hits + 1)

    scores = myProbabs + kExp * exploratio_score  + 0.1 / (losses+1)
    scores[(my_hits+his_hits) > max_hit] = -20

    if (my_hits > 0 ).sum() > 20 and step < 1998 :
        scores[my_record[step-2:step]] = -2 # not good for the begining!
