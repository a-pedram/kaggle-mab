...
def new_bandit(step):
    ...
    if step == 1 :  choicePath = {'m1':0, 'm2':0, 'm3':0, 'm4':0,'m5':0, 'm6':0,'exploit':0}
    ...
    my_best_bests = np.where((losses * 2.5 < wins) & (my_hits >2))[0]
    if my_best_bests.shape[0] >2 :
        choicePath['exploit'] += 1
        steps = np.zeros(my_best_bests.shape[0] )
        for i in range( my_best_bests.shape[0] ):
