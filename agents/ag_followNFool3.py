...
def new_bandit(step):
    ...
    if last_reward == 1:
        return last_bandit
    if n_last_move >= 2 and myProbabs[his_last_bandit] > 0.12 and (my_hits[his_last_bandit]+his_hits[his_last_bandit]) < max_hit:
    ...
    scores = (wins - losses + his_hits -( his_hits > 0) ) / (wins +losses + his_hits) * 0.97**(wins+losses+his_hits)
    # scores = myProbabs + kExp * exploratio_score  + 0.1 / (losses+1)
    scores[(my_hits+his_hits) > max_hit] = -np.inf

    # scores[my_record[step-5:step]] = -2
...
def agent(obs, conf):
    ...
    global last_bandit, last_reward, total_reward,  record_index, wins, losses, his_rec_index, his_bandit_rec
    if obs.step == 0:        
        total_reward = 0 
