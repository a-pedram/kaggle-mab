...
def new_bandit(step):
    ...
    if n_last_move >= 2 and n_last_move > my_hits[his_last_bandit]:
        #
        # TODO
        # Make this conditional when you have a good info about it for instance when
        # you have more than 5 hits and it is not in your best 5 or 10
    ...
    if step > 100:
        his_100_ago = his_record[step - 100]
        n_100_ago = (his_record[step - 100:step] == his_100_ago).sum()
        if n_100_ago == 1 :
            myProbabs[his_100_ago] -= .25 
            #his_low_chances.append(his_100_ago)
    ...
    scores = myProbabs + kExp * exploratio_score
    scores = myProbabs + kExp * exploratio_score + kHisSco * his_scores + .05/(losses+1)

    scores[(my_hits+his_hits) > max_hit] = -20
    if his_last_bandit in np.argsort(scores)[-20:0] :
        winner = his_last_bandit
    else:
        winner = np.argmax(scores)
    # if step > 1400:
    #     winner = int(np.argmax(myProbabs))
    return winner
